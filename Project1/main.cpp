...
bool playTurn(vector<Player<string>>& players, Player<string> &player, 
              Environment &env);
...
int main() {
    ...
    while (!gameOver) {
        ...
        if (playTurn(players, currentPlayer, env)) {
    ...
    if (choice == 0) {
        ...
        if (selectedCard.number == "Draw Four" || 
            ...
            if (selectedCard.number == "Wild Draw Four") {
                ...
                int nextPlayerIndex = env.getCurrentPlayerIndex();
                Player<string>& nextPlayer = players[nextPlayerIndex];
        ...
        if (selectedCard.number == "Draw Two" && 
           (selectedCard.color == topCard.color || 
            ...
            env.advancePlayer(env.getCurrentPlayerIndex());
            Player<string>& nextPlayer = players[env.getCurrentPlayerIndex()];
        ...
        if (selectedCard.color == topCard.color 
           || selectedCard.number == topCard.number) {
            ...
            return playTurn(players, player, env);
        }
    }
